...
async def fetch_buids_subflow(
    ...
    for i, result in enumerate(ps_results):
        if isinstance(result, Exception):
            logger.error(f"❌ PSQuery {i} failed: {type(result).__name__}: {result}")
        else:
            student_buid_set.update(result)

    for i, result in enumerate(sap_results):
        if isinstance(result, Exception):
            logger.error(f"❌ SAP query {i} failed: {type(result).__name__}: {result}")
        else:
    ...
    for i, result in enumerate(vds_results):
        ...
        if isinstance(result, Exception):
            logger.error(f"❌ VDS [{affiliation}] failed: {type(result).__name__}: {result}")
